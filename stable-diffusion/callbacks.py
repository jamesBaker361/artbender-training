...
class GenImgCallback(keras.callbacks.Callback):
    def __init__(self,model, save_folder, prompts=["man holding a sword walking at night", "two birds on a branch", "woman playing with cat"], images_to_generate=2, *args, **kwargs):
...
class SaveModelCallback(keras.callbacks.Callback):
    def __init__(self, model,save_model_folder) -> None:
        super(SaveModelCallback, self).__init__()
        self.model=model
        self.save_model_folder= save_model_folder

    def on_epoch_end(self,epoch,logs=None):
        tf.saved_model.save(self.model,self.save_model_folder)
